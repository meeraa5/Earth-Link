    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch('/api/events', {
                ...
                body: JSON.stringify({ title: title.trim(), location: location.trim(), description: description.trim(), eventTime })
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || 'Failed to create event');
            onEventCreated(data.event);
            setTitle(''); setLocation(''); setDescription(''); setEventTime('');
            onClose();
        } catch (err) {
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm text-stone-600">Title</label>
                            <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full mt-1 p-2 border rounded" />
                        </div>

                        <div>
                            <label className="block text-sm text-stone-600">When</label>
                            <input type="datetime-local" value={eventTime} onChange={(e) => setEventTime(e.target.value)} className="w-full mt-1 p-2 border rounded" />
                        </div>
                        ...
                        <div>
                            <label className="block text-sm text-stone-600">Description (optional)</label>
